...
class ElectronicVoucher(models.Model):
    ...
    def _get_digital_certificate(self):
        ...
        :return:
        """
        object = self.env['sri.digital.certificate']
        digital_certificate = object.search([
    ...
    def _get_vals_information_company(self):
        ...
        return {
            ...
            'codDoc': code,
            'estab': serie.shop_id.establishment,
    ...
    def _update_document(self, data):
        ...
        date = datetime.strptime(authorization['fechaAutorizacion'], DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(
            hours=5)
    ...
    def _get_json(self, certificate):
        ...
        data_json.update({
            'info': {
                ...
                'detail': self.document_id.name or 'N.A' if code in ['18', '04'] else '-',
                'obligado': "SI",
                'especialn': "-",
    ...
    def _get_url_api(self):
        ...
        return self.env['ir.config_parameter'].sudo().get_param(
            ...
            default='http://facturero.elitumerp.com/api/apiofflinesri'
        )
    ...
    def button_generated_api(self):
        ...
        self.filtered(lambda d: d.state in ['new', 'not_authorized_sri'])
        for document in self:
            certificate = document._get_digital_certificate()
            if not certificate:
                document.write({'comment': utils.MESSAGE_CERTIFICATE})
                continue
            requestBody = json.dumps(document._get_json(certificate[0]))
            try:
