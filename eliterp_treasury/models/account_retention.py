...
class Invoice(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self.invoice_line_ids:
            ...
            for tax in line.invoice_line_tax_ids:
                reference = tax.tax_group_id.tax_reference
                if reference == 'i':
    ...
    base_no_iva = fields.Float('No objeto IVA', currency_field='currency_id', compute='_compute_amount',
                                 store=True)
...
class Retention(models.Model):
    ...
    def confirm_retention(self):
        ...
        customer_retention_account_id = self.company_id.customer_retention_account_id
        if not customer_retention_account_id:
            raise UserError(_("No tiene configurada en la compañía la cuenta de retención (cliente)."))
        line_move_retention = self.env['account.move.line'].with_context(check_move_validity=False).create({
            'name': '/',
            'journal_id': journal_id.id,
            'partner_id': self.partner_id.id,
            'account_id': customer_retention_account_id.id,
            'move_id': move_id.id,
            'credit': self.total,
            'debit': 0.0,
            'date': self.date_retention
