...
class Ats(models.TransientModel):
    ...
    def _get_canceled(self, period):
        ...
        for inv in self.env['account.invoice'].search(domain):
            ...
            detalleanulados = {
                'tipoComprobante': '18',
                'establecimiento': point_printing.shop_id.establishment,
    ...
    def _get_sales(self, period):
        ...
        for ruc, group in groupby(sales, key=itemgetter('idCliente')):
            ...
            detalle = {
                ...
                'idCliente': ruc,
                'parteRel': 'NO',
                'tipoComprobante': '18' if inv.type == 'out_invoice' else '04',
    ...
    def _get_refund(self, invoice):
        """
        TODO: Obtenemos la nota de crédito de la factura (Probar)
        :param invoice:
        :return:
        """
        return {
            'docModificado': '01',
            'estabModificado': invoice.serial_number[:3],
            'ptoEmiModificado': invoice.serial_number[4:],
            'secModificado': invoice.invoice_number,
    ...
    def generate_ats(self):
        ...
        ats.numEstabRuc = self.shop_id.establishment
...
class TaxesSummary(models.TransientModel):
    _name = 'sri.103.104.summary'
    _inherit = ['report.xlsx.abstract', 'sri.ats']
    _description = _("Ventana para impuestos 103, 104")
    ...
    def _get_lines_sales(self, context):
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line_ids:
                ...
                register.append(0.00)  # Base Exento
                ...
                for tax in line.invoice_line_tax_ids:
                    ref = tax.tax_group_id.tax_reference
                    if ref == 'i':
            ...
            count = -1
            for r in rent:
                data[count][16] = r.tax_id.code if r.tax_id.code else "-"
                data[count][17] = str(int(r.tax_id.amount)) + '%' if r.tax_id.amount else "-"
                data[count][18] = r.amount
                count = count - 1
            count = -1
            for i in iva:
                data[count][20] = i.tax_id.code if i.tax_id.code else "-"
                data[count][21] = str(int(i.tax_id.amount)) + '%' if i.tax_id.amount else "-"
                data[count][22] = i.amount
                count = count - 1
            data[-1][23] = invoice.amount_total
        return data
    ...
    def _get_lines_purchases(self, context):
        ...
        for invoice in invoices:
            ...
            for r in rent:
                data[count][16] = r.tax_id.code if r.tax_id.code else "-"
                data[count][17] = str(int(r.tax_id.amount)) + '%' if r.tax_id.amount else "-"
                data[count][18] = r.amount if invoice.state != 'cancel' else 0.00
                count = count - 1
    ...
    def generate_xlsx_report(self, workbook, context):
        ...
        columns = [
            ...
            '# DOCUMENTO', 'AUTORIZACIÓN', 'S. TRI.', 'B. IVA', 'B. CERO', 'ICE', 'B. NO IVA', 'B. E. IVA',
    ...
    def print_report_xlsx(self):
        ...
        self.write(self.create_xlsx_report('Impuestos 103, 104', context))
        return {
            'name': "Impuestos 103, 104",
            'type': 'ir.actions.act_window',
            'res_model': 'sri.103.104.summary',
            'view_mode': ' form',
            'view_type': ' form',
            'res_id': self.id,
            'views': [(False, 'form')],
