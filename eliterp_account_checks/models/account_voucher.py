...
class Voucher(models.Model):
    ...
    def _other_actions_to_cancel(self):
        res = super(Voucher, self)._other_actions_to_cancel()
        if self.type_egress == 'check':
            check = self.env['account.checks'].search([('voucher_id', '=', self.id)])
            if check:
                check.update({'state': 'protested'})
        return res

    def _get_move_name(self):
        move_name = super(Voucher, self)._get_move_name()
        if self.type_egress == 'check':
            prefix = 'COEG-{0}-{1}-{2}'.format(
                self.company_id.name[:3],
                self.bank_journal_id.code,
                str(self.bank_date.year)
            )
            move_name = '{0}-{1}'.format(
                prefix,
                self.check_number
            )
        return move_name
    ...
    def _onchange_check_number(self):
        """
        Rellenamos con 0 el número de cheque, pendiente hacerlo secuencial
        :return:
        """
