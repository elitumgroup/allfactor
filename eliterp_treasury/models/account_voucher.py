...
class Voucher(models.Model):
    ...
    def _get_move_name(self):
        ...
        if self.type_egress == 'transfer':
            move_name = 'NDBA-{0}-{1}'.format(
                self.company_id.name[:3],
                self.bank_journal_id.sequence_id.next_by_id()
            )
        return move_name
    ...
    reference = fields.Char(string='Concepto', readonly=False,
                            help="Colocar alguna referencia, nota del comprobante.", copy=False)  # CM
