...
class ContractType(models.Model):
    ...
    halftime = fields.Boolean('Medio tiempo', default=False,
                              help='Colocar verdadero si tipo de contrato es medio tiempo.')
...
class PayslipRun(models.Model):
    ...
    def confirm_payslip_run(self):
        ...
        for role in self.line_ids:  # Comenzamos a sumar los roles individuales para creación del consolidado
            ...
            if role.role_id.employee_id.contract_id.type_id.halftime:
                sstp += round((float(role.wage) * 0.04408), 3)  # Medio tiempo
            ...
            if role.role_id.struct_id.provisions:  # Para no colocar nada a las ES sin provisiones
                ...
                if role.role_id.employee_id.working_time or role.role_id.employee_id.id == 86:
                    if role.role_id.employee_id.accumulate_reserve_funds == 'yes':
                        # Fondos de reserva retenidos
                        flag_reserve_funds = True
                        provision_reserve_funds += round((float(role.wage) * float(8.33)) / float(100), 3)
                    # Fondos de reserva cobrados (Gastos)
                    reserve_funds += role.reserve_funds
        reserve_funds = reserve_funds + provision_reserve_funds  # Gasto van las dos
        # Décimos
        ...
        self._create_line_expenses('SS_TP', journal_id, move_id, sstp)  # SSTP
        ...
        self._create_line_income('SS_TPG', journal_id, move_id, sstp, False)  # SSTP (Gastos)
        ...
        if not roles:
            ...
            for role in roles:
                data = {
                    ...
                    # Ingresos
                    'wage': role.input_line_ids.filtered(lambda x: x.code in ['SUE', 'SUE_MT'])[0].amount,
                    ...
                    'iess_personal':
                        role.input_line_ids_2.filtered(lambda x: x.code in ['IESS_9.45%', 'IESS_9.45%_MT'])[
                            0].amount if role.input_line_ids_2.filtered(
