...
class SriPointPrinting(models.Model):
    ...
    @api.depends('shop_id', 'emission_point')
    def _compute_name(self):
        for record in self:
            record.name = "{0} - {1}".format(record.shop_id.establishment, record.emission_point)
...
class SriAuthorization(models.Model):
    ...
    authorization = fields.Char('Nº Autorización', required=True)
